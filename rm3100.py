...
class RM3100(object):
    ...
    def configure_the_RM3100(self):
        ...
        time.sleep(0.02)
        self.write([self.RM3100_CCXLSB_REG, self.CCP1, self.CCP0,
                    self.CCP1, self.CCP0, self.CCP1, self.CCP0])  # initial Config
        time.sleep(0.02)
        self.write([self.RM3100_CMM_REG, 0x79])  # write to CMM
        time.sleep(0.02)
        # write to TMRC to be 150Hz
        self.write([self.RM3100_TMRC_REG, self.RM3100_TMRC_150HZ])
        time.sleep(0.02)
        TMRC = self.read([0x8B])
        CMM = self.read([0x81])
    ...
    def send_Poll_Read(self):
        # send Poll Reg
        self.write([self.RM3100_POLL_REG, self.RM3100POLL_MXYX])
        time.sleep(0.01)
